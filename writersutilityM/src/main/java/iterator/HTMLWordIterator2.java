...
public class HTMLWordIterator2 extends WordIterator {
	...
	private int minLength = 0, fragmentIndex = 0;
	private Pattern pattern = Pattern.compile("([A-Za-zÀ-ÿ]+(&#(?:19[2-9]|2[0-4][0-9]|25[0-5]);)?)");
	private Matcher matcher = pattern.matcher(text);
	private Fragment currentFragment;
	private int findIndex;

	public HTMLWordIterator2(FragmentIterator fragmentIterator) {
	...
	public boolean hasNext() {
		if(currentFragment==null && !fragmentIterator.hasNext()) {
			return false;
		}
		if(currentFragment==null && fragmentIterator.hasNext()) {
...
//			word = new Word(matcher.group(),currentFragment.getIndex()+matcher.start());
